...
function mutateRanksToAlphabetize(imported) {
  // Compute the absolute overall rank of each import, first by group, then by
  // import name. Mutate the importedItem.group to reflect its final rank.
  ...
  const importedItemRanks = Object.keys(importedItemNamesByRank).sort()

  // It's not important that the item ranks are sorted here, just convenient.
  importedItemRanks.forEach(function (rank) {
    // Use `sort` to mutate the array of import names
    importedItemNamesByRank[rank].sort(function (a, b) {
      return a.localeCompare(b)
    })
  })

  // Build up the list of absolutely-sorted import names, group by group
  const absoluteSortedItemNames = importedItemRanks.reduce(function (acc, rank) {
    return acc.concat(importedItemNamesByRank[rank])
  }, [])

  // Mutate the importItem.rank to its absolute rank
  imported.forEach(function (importedItem) {
    importedItem.rank = absoluteSortedItemNames.indexOf(importedItem.name)
  })
}
